...
```json
{
  ...
  "video_prompt": "Xiaomei (a 25-year-old Asian woman with long black hair) looks up at the server, smiling gently and says, 'It's really quiet here, I like it.' Clear, pleasant voice, moderate pace. Keep 9:16 vertical composition. No background music. Natural ambient sound only.",
  "dialogue": {
    "speaker": "Xiaomei",
...
| `no_bgm` | boolean | Whether to add "No background music" constraint in prompt, default true |

### BGM Decision Logic

BGM is mixed in post-production, not generated during video generation. The `no_bgm` field controls whether to add constraint in prompt:

**Decision rules (based on `creative.json`'s `bgm.type`)**:
- `bgm.type = "ai_generated"` → All shots `no_bgm = true` (BGM generated by Suno, mixed in post)
- `bgm.type = "user_provided"` → All shots `no_bgm = true` (BGM provided by user, mixed in post)
- `bgm.type = "none"` → All shots `no_bgm = false` (video model decides freely)

**When `no_bgm = true`, add at the end of video_prompt**:
```
      ...
      "shots": [
        {
          ...
          "video_prompt": "Interior of a cozy city coffee shop, afternoon sunlight streaming through large floor-to-ceiling windows, slow push in. Keep 9:16 vertical composition. No background music. Natural ambient sound only.",
        ...
        },
        {
          ...
          "generation_mode": "img2video",
          "video_prompt": "Xiaomei (a 25-year-old Asian woman with long black hair) sits by the coffee shop window, smiling gently and says, 'It's really quiet here, I like it.' Clear, pleasant voice, moderate pace. Keep 9:16 vertical composition. No background music. Natural ambient sound only.",
